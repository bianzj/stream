...
void biochemical_Farquhar(Canopy canopy, LeafBio leafbio, SoilSet soilset, Thermal thermal, NetRad netrad, BioState &biostate, Meteo meteo, MeteoMeta meteod)
{
    //使用了 Farquhar 模型来描述 C3 和 C4 植物的光合作用。
    ...
    }else
    {
        ...
        for(int issunlit=0; issunlit<2; issunlit++)
        {
            ...
            Es = biostate.es;
            // 净辐射，光合有效辐射（PAR）
            Q = netrad.directPrad_leaf + netrad.diffusePrad_leaf;
            // 叶片边界层中的初始水蒸气压
            eb = meteod.ea;
            // 氧气浓度
            O = meteod.Oa;
            ...
            // Ball-Berry 截距 'b'
            BallBerry0 = 0.01;
            // 每个电子的 CO2 数量（通常 C3 为 1/5，C4 为 1/6）
            ...
            //-------------------------------------
            float Tref = 25 + 273.15; // 25°C 的绝对温度
            float Kc25 = 350;         // CO2 的动力学系数
            float Ko25 = 450;         // O2 的动力学系数
            float spfy25 = 2600;      // 特异性
            float Kpep25 = (Vcmax25 / 56) * 1e6; // PEPcase CO2 速率常数，C4 植物使用
            float atheta = 0.8;       // 光化学效率
            // 电子传输和荧光速率常数
            float Kf = 0.05;          // 荧光速率常数
            float Kd = Utils::max(0.8738, 0.0301 * (Tc - 273.15) + 0.0773); // 热解活化速率常数
            float Kp = 4.0;           // 光化学速率常数
            float rhoa = 1.2047;      // 空气密度
            float Mair = 28.96;       // 干空气的分子质量
            // 将单位转换为 bar
            ...
            //-------------------------------
            float RH,Ci;
            RH = Utils::min(1,eb/ SCI::es_fun(Tc-273.15));
...
void biochemical_M12(Canopy canopy, LeafBio leafbio, SoilSet soilset, Thermal thermal, NetRad netrad, BioState &biostate, Meteo meteo, MeteoMeta meteod)
{
    ...
    }else if(type ==1 )
    {
        ...
        if(thermal.Tleafsunlit < 100)
        {
            Tsunlit = thermal.Tleafsunlit +273.15;
            Tshaded = thermal.Tleafshaded +273.15;
        }else
        {
            Tsunlit = thermal.Tleafsunlit ;
            Tshaded = thermal.Tleafshaded ;
        }
        ...
        for(int kt = 0;kt<2;kt++)
        {
            ...
            cs = biostate.cs * p * 1e-11;
            O = O * p * 1e-8;
            Q = netrad.directPrad_leaf;
            ...
            CRD = 1000 * HARD / (RGAS*TREF);                   // []             scaling factor in RD response to voxelTempe

            HAGSTAR = 37.83;                                 // [kJ/mol]       activation energy of Gamma_star
            CGSTAR = 1000 * HAGSTAR / (RGAS*TREF);                // []             scaling factor in GSTAR response to voxelTempe
            ...
            {
                // C3// species
                CKC = 1000 * HAKC / (RGAS*TREF);                     // []             scaling factor in KC response to voxelTempe
                Kc = KCOP*exp(CKC - HAKC / dum1)*1e-11*p;     // [bar]          Michaelis constant of carboxylation adjusted for voxelTempe (Bernacchi et al. 2001)

                CKO = 1000 * HAKO / (RGAS*TREF);                     // []             scaling factor in KO response to voxelTempe
            ...
            if (Type == 0)                                           // C3 species, based on Farquhar model (Farquhar et al. 1980)
            {
                ...
                W = Utils::min(Wc, Wj);                                // [umol/m2/s]       carboxylation rate
            ...
            else                                             // C4 species, based on von Caemmerer model (von Caemmerer 2000)
            {
                ...
                Vp = Utils::min(Vpc, Vpr);                            // [umol/m2/s]       PEP carboxylation rate
                ...
                if (Ac != Ac && Aj == Aj) A = Aj;
                if (Aj != Aj && Ac == Ac) A = Ac;
                if (Ac == Ac && Aj == Aj)  A = Utils::min(Ac, Aj);                             // [umol/m2/s]       net photosynthesis
            ...
            if(kt==0) biostate.rsshaded = rss;
            else biostate.rssunlit= rss;

            if(!isnan(Ci)) biostate.ci = float(Ci * 1e6 / p *1e5);
...
void BioChemical::suresist(std::shared_ptr<Defined> defined, std::shared_ptr<PixelIO> pixelio) {

    Canopy &canopy = pixelio->m_pInputset->canopy;
    AeroCoeff &aerocoeff = defined->m_aerocoeff;
    ...
    LeafBio &leafbio = pixelio->m_pInputset->leafbio;
    SoilSet &soilset = pixelio->m_pInputset->soilset;
    ...
    if(canopy.lai >=0) {
        biochemical_Farquhar(canopy, leafbio, soilset, thermal, netrad, biostate, meteo, meteod);
    }
}


void BioChemical::suresist_urban(std::shared_ptr<Defined> defined, std::shared_ptr<PixelIO> pixelio) {
