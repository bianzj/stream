...
//    double trueSolarTime = localtime;
...
//    double hourAngle = trueSolarTime / 4.0 - 120.0;
...
//void Geometry::calcSolarAngle(std::shared_ptr<Defined> defined,std::shared_ptr<PixelIO> pixelio) {
//
//    double lat = pixelio->lat;
//    double lon = pixelio->lon;
//    int knode = pixelio->k_node;
//    double t = pixelio->m_pInputset->vMeteo[knode].t;
//    double minutes = t*60*24.0;  //将时间t转换为分钟数
//    double T = t+defined->m_doy; // 将时间 t 加上 defined->m_doy（表示一年中的天数）
//
////    T = T + 8 / 24.0;
////    float sza = 0,saa = 0;
////    Utils::sunAngle(T,lon,lat,&sza,&saa);
//
////    calcAzEl(T,minutes,lat,lon,0);
//    calcAzEl(T,minutes,lat,lon,0);
//    pixelio->m_angle.sza = sza;
//    pixelio->m_angle.saa = saa;
//    pixelio->m_angle.vza = 0;
//    pixelio->m_angle.vaa = 0;
//
//}
...
void Geometry::calcSolarAngle(std::shared_ptr<Defined> defined,std::shared_ptr<PixelIO> pixelio) {
    ...
    float p = pixelio->m_pInputset->vMeteo[knode].p;
    float dem = pixelio->m_pInputset->terrain.dem;
    float slope = pixelio->m_pInputset->terrain.slope;
    float ta = pixelio->m_pInputset->vMeteo[knode].ta;

    int kmonth = 0;
    int kday = 0;
    calculateMonthAndDay(defined->m_year, defined->m_doy, &kmonth, &kday);

    SPACalc spa;
    spa_data data;
    data.year = defined->m_year;
    data.month = kmonth;
    data.day = kday;
    data.hour = knode;
    data.minute = 0;
    ...
    data.timezone = 0.0;
    data.pressure = p;
    data.temperature = ta - 273.15;
    data.delta_t = calculateDeltaT(data.year, data.month);
    data.longitude = lon;
    data.latitude = lat;
